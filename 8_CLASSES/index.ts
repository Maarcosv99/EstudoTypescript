...
class NewUser {
  name // o tipo será por inferência
  age // o tipo será por inferência
...
class Car {
  name
  readonly wheels = 4 // tipo está por inferência
...
class Coords {
  // Colocamos a " ! " pq não utilizaremos o constructor
...
/*
  ...
  Porém esta forma é mais utilizada para criar os métodos que várias
  classes terão em comum.
*/

interface showTitle {
  itemTitle(): string
  // itemTitle?(): string -> Deixaria o método opcional.
}
...
class StaticMembers {
  ...
  static prop: string = "Teste static"
